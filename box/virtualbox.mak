...
install_os_vm_nodeps.vbx: create_vm.vbx
	@bash $(BOX_DIR)/host/virtualbox/install_os_vm_vbx.sh $(VM_NAME) "$(BOX_DIR)/host/virtualbox/work" "$(INSTALL_MEDIA_DIR)/$(VM_INSTALL_MEDIA_FILE)" $(VM_HOSTNAME) $(VM_USER) $(VM_PASSWORD)
#	@bash $(BOX_DIR)/host/virtualbox/install_os_vm_vbx.sh -d $(SRC_LOG_DIR)/virtualbox $(VM_NAME) "$(BOX_DIR)/host/virtualbox/work" "$(INSTALL_MEDIA_DIR)/$(VM_INSTALL_MEDIA_FILE)" $(VM_HOSTNAME) $(VM_USER) $(VM_PASSWORD)
...
.PHONY: remove_vm.vbx
remove_vm.vbx:
	@-bash $(BOX_DIR)/host/virtualbox/remove_vm_vbx.sh $(VM_NAME)

# Remove work dir
.PHONY: work_clean.vbx
work_clean.vbx:
	@-rm -rf $(BOX_DIR)/guest/virtualbox/work
